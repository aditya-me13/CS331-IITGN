 ...
 */
int directCosts3[4] = { 7, 999, 2, 0 };  /* Direct link costs to each node */
 ...
 */
int findMinimum3(int values[4]) {
    int minVal = INFINITY;
    for (int i = 0; i < 4; i++) {
        minVal = getMin3(minVal, values[i]);
    }
    return minVal;
}
...
void updateMinimumCosts3() {
    ...
    for (int dest = 0; dest < 4; dest++) {
        ...
        for (int via = 0; via < 4; via++) {
            ...
            if (via != 1) {
                bestCost3[dest] = getMin3(bestCost3[dest], dt3.costs[dest][via]);
            }
        }
    }
}
...
void broadcastUpdates3() {
    ...
    for (int neighID = 0; neighID < 4; neighID++) {
        outPackets3[neighID].sourceid = NODE_ID;
        outPackets3[neighID].destid = neighID;
    ...
    for (int neighID = 0; neighID < 4; neighID++) {
        if (neighID != NODE_ID && neighID != 1 && directCosts3[neighID] < INFINITY) {
...
void rtinit3() {
    ...
    for (int dest = 0; dest < 4; dest++) {
        for (int via = 0; via < 4; via++) {
            if (dest == via) {
                /* Direct link costs */
                dt3.costs[dest][via] = directCosts3[dest];
            } else {
...
void linkhandler3(int linkid, int newcost) {
    ...
    int oldDirectCost = directCosts3[linkid];
    directCosts3[linkid] = newcost;
